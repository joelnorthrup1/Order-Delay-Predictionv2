...
def merge_encode(file_dir,training_set):
        ...
        X,y=preprocess(dataset,"X5_train","y5_train",training_set)
        return X,y
...
def inference():
        context = {}
        dataset=pd.read_csv("../../Project3/dataset1.csv",skiprows=lambda i: i>0 and random.random() > 0.01).reset_index()
        print("Recieved inputs")
        context = {}
        X_processed,y_processed=preprocess(dataset,"X_test","y_test",False)
        X,y,date=preproces_data(5,500,training_set=False,with_label=True)
        ...
        X_processed=X_processed.drop('time',axis=1)
        col=['EM_ITEM_NUM','GNRC_NAME','THERAP_CLASS','FD Location','API Location','DC_NUM','SPLR_VENDOR_NUM','SPLR_NAME','SPLR_REGION','SPLR_POSTAL_CODE','SPLR_COUNTRY']
        X_processed=inverse_encoded_data(X_processed,col)
        final=pd.concat([pd.DataFrame(final),pd.DataFrame(X_processed)],axis=1)         
        final.to_csv("./MyApp/inferences28.csv")
        #return render(request, 'home/home.html', context)

def shap():
    days_delayed_thres=12
...
#merge_encode("../../Project3/dataset1.csv",training_set=True)
#training()
inference()
#shap()
