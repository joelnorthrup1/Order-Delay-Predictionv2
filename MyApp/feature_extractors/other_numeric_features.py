...
def date_specific(dataset1,risk,feature_name):
  ...
  #print(risk['time'])
  try:
    merged_dataframe = pd.merge_asof(dataset1.sort_values('time'),risk[['time','Value']].sort_values('time'),on='time',direction='nearest',tolerance=pd.Timedelta('1y'))
    merged_dataframe.rename({'Value': feature_name}, axis=1, inplace=True)
  except:
    merged_dataframe = pd.merge_asof(dataset1.sort_values('time'),risk[['time','GPRHC_IND']].sort_values('time'),on='time',direction='nearest',tolerance=pd.Timedelta('1y'))
