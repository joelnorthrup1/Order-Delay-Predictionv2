...
PATH_TO_REPO='../'
# pollution has columns equal to ports and rows as daily visibility score
#risk=pd.read_csv("./ext_data/geo_political_risk.csv").reset_index()
...
# def replace(indices):
#     try:
#       value=risk.iloc[[int(indices)],:]['GPRHC_IND']
#     except:
#       value=np.nan
...
def geo_political_risk(df,risk):
    date1=[]
    #df['XPCT_DLVRY_DT'].apply(date_string)
    #df=df.dropna()
    for i in range(len(risk)):
        rsd=datetime.datetime.strptime(risk['ReportTime'][i][:10],'%Y-%m-%d')
        date1.append(rsd)
    date2=[]
    ...
    merged=pd.concat([pd.DataFrame(df).reset_index(),risk.loc[indices['indices'],:]['GPRHC_IND'].reset_index()],axis=1)
