# -*- coding: utf-8 -*-
"""
Created on Fri Nov 25 17:03:20 2022
...
def date_specific(dataset1,risk,feature_name):
  #print(risk.columns)
  #print(risk)
  ...
  #print(risk['time'])
  try:
    merged_dataframe = pd.merge_asof(dataset1.sort_values('time'),risk[['time','Value']].sort_values('time'),on='time',direction='forward',tolerance=pd.Timedelta('1y'))
    merged_dataframe.rename({'Value': feature_name}, axis=1, inplace=True)
  except:
    merged_dataframe = pd.merge_asof(dataset1.sort_values('time'),risk[['time','GPRHC_IND']].sort_values('time'),on='time',direction='forward',tolerance=pd.Timedelta('1y'))
...
def merge_for_other_features(dataset,risk,feature_name):
    ...
    processing_data=date_specific(processing_data,risk,feature_name)
