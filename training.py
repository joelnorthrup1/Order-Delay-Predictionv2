...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--delay", required=True, help="CSV file with quotes to run the model")
...
def spreproces_data(X,days_delayed,pcs_components,ss,training_set,with_label=True):
  if(training_set==True):
    ...
    y=(pd.read_csv("./y_test100_processed.csv",engine='python')['Days_Delayed']>days_delayed).astype(np.int8)
...
X=pd.read_csv("./X_test100_processed.csv")
X1=X.iloc[:,21:351]
X2=X.iloc[:,351:2000]
X3=pd.concat([X1,X2],axis=1)
#X=X.iloc[:,:-1]
X_train, X_test, y_train, y_test=spreproces_data(X3,int(args['delay']),None,"./standardscaler3m.obj",training_set=True)
